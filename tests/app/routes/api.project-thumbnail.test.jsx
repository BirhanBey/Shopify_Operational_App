...
describe('api.project-thumbnail.jsx', () => {
  let loader;
  let mockPrisma;
  ...
  beforeEach(async () => {
    ...
    // Import after mock
    const module = await import('../../../app/routes/api.project-thumbnail.jsx');
  ...
  it('should return error when projectId is missing', async () => {
    const request = new Request('http://localhost/api/project-thumbnail?shop=test-shop.myshopify.com');
  ...
  it('should return error when shop is missing', async () => {
    const request = new Request('http://localhost/api/project-thumbnail?projectid=project-123');
    const response = await loader({ request });
    const data = await response.json();
  ...
  it('should return error when editor settings are not configured', async () => {
    ...
    const request = new Request('http://localhost/api/project-thumbnail?projectid=project-123&shop=test-shop.myshopify.com');
    const response = await loader({ request });
    const data = await response.json();
  ...
  it('should fetch and return thumbnail as base64', async () => {
    mockPrisma.editorSettings.findUnique.mockResolvedValue(mockEditorSettings);

    // Mock image response
    const mockImageBuffer = new Uint8Array([137, 80, 78, 71]); // PNG header
    ...
    expect(data.thumbnail).toContain('data:image/png;base64,');
  ...
  it('should handle API errors', async () => {
    ...
    expect(data.error).toContain('Thumbnail API error');
  });
  ...
  it('should handle fetch errors', async () => {
    ...
    expect(data.error).toBe('Failed to fetch thumbnail');
  });

  it('should include CORS headers', async () => {
    mockPrisma.editorSettings.findUnique.mockResolvedValue(mockEditorSettings);

    const mockImageBuffer = new Uint8Array([137, 80, 78, 71]);
    global.fetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      headers: new Headers({ 'content-type': 'image/png' }),
